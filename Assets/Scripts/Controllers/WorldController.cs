...
#pragma warning restore 649
        ...
        /// <summary>
        /// Remove visited grid blocks from the list
        /// </summary>
        /// <param name="visitedId"></param>
        public void OnVisit(int visitedId)
        {
            for (int t = 0; t < unVisitedGridBlocks.Count; t++)
            {
                if (unVisitedGridBlocks[t].id == visitedId)
                {
                    unVisitedGridBlocks.RemoveAt(t);
                    if (unVisitedGridBlocks.Count == 0)
                    {
        ...
        /// <summary>
        /// Returns Pizza Position from Unvisited List of Grid Blocks
        /// </summary>
        /// <returns></returns>
        public Vector3 GetPizzaPosition()
        {
            if (unVisitedGridBlocks.Count > 0)
            {
                int index = UnityEngine.Random.Range(0, unVisitedGridBlocks.Count);
                return unVisitedGridBlocks[index].gameObject.transform.position;
            }
            return Vector3.zero;
        }
